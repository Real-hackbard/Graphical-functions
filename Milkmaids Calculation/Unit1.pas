...
uses
  ...
  Menus, Buttons, ComCtrls, StdCtrls, ExtCtrls, XPMan;
...
var Gross,Mittel,Klein,
    Anzahl,loesung1,loesung2:Integer;       // Number of transfers
    VollGross,VollMittel,VollKlein,         // Capacities
    AnfangGross,AnfangMittel,AnfangKlein,   // Initial values
    EndeGross,EndeMittel,EndeKlein:Integer; // Final values
    erfolg:boolean;

procedure Umgiessen(var Krug1,Krug2,Krug3:Integer);
// It is poured from jug 1 into jug 2, taking into account the capacity of jug 3.
begin
   inc(Anzahl);
...
begin
   ...
   Ausgeben;
   Anzahl:=0;
   // Large --> Small --> Medium --> Large, think of it as a triangle!!!
   repeat
            ...
            begin
              ...
              if umf>0 then
                lbox1.items.add('('+inttostr(u)+','+inttostr(v)+','+inttostr(w)+')'+
                               #9+inttostr(umf)+' Transfers');
            end;
        end;
      end;
    end;
    end;
    if lbox1.items.count=0 then lbox1.items.add('no possible solution');
    liste1.free;
    liste2.free;
    paintb1paint(sender);
end;
