{
  ...
  every operator follows all of its operands, in contrast to Polish notation,
  which puts the operator in the prefix position. It is also known as
  Postfix notation and is parenthesis-free as long as operator arities
  are fixed.

  The description "Polish" refers to the nationality of logician Jan ?
  ukasiewicz, who invented (prefix) Polish notation in the 1920s."
...
{   Component(s):
    ...
    Description:
    Math parsing component that can evaluate an expression using
    common operators, some functions and variables
...
procedure TMathParser.StrToInfixStack(aExpression: String; aStack: TStack);
var
...
function TMathParser.ValidateInfixStack(aStack: TStack): Boolean;
var
...
procedure TMathParser.InfixToPreFix(Infix: TStack; Prefix: TStack);
var
  TmpStack: TStack;
  i: Integer;
  Current: TStackInfo;
...
function TMathParser.GetParserResult: Extended;
begin
...
function TMathParser.GetLastError: Integer;
begin
  Result := fLastError;
end;
