...
begin
  ...
  if p=0 then {invalid function call} fehler:=1;
...
begin
  ...
  case funktionscode of
   ...
   43: begin
         a:=gesamtkette;
         if a>0 then funktion:=0.4342944819033*ln(a) {function:=log10(a)}
         else begin
           funktion:=0;
           fehler:=2;
         end;{LG(A<=0) not allowed}
       end;
        ...
        IF x <> 0.0 THEN
          ...
          ELSE
            if abs(y*ln(abs(x))) > 15.0 THEN begin
              x_hoch_y:=0;
              fehler:=2
            end else x_hoch_y := -Exp(Ln(ABS(x)) * y) (* odd exponent *)
        ELSE
          x_hoch_y := 0
      END ELSE
        x_hoch_y := 1.0
