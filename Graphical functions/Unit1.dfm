object Form1: TForm1
  Left = 183
  Top = 90
  Width = 1058
  Height = 745
  HelpContext = 100
  Caption = 'Graphical representation of functions'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 13
  object Panel5: TPanel
    Left = 0
    Top = 0
    Width = 1042
    Height = 686
    Align = alClient
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 0
    object Panel6: TPanel
      Left = 824
      Top = 0
      Width = 218
      Height = 686
      Align = alRight
      BevelOuter = bvNone
      Color = 15790320
      TabOrder = 0
      object Panel8: TPanel
        Left = 0
        Top = 0
        Width = 218
        Height = 70
        Align = alTop
        BevelOuter = bvNone
        Color = 15790320
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object slinks: TSpeedButton
          Left = 18
          Top = 33
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            22002222222222222200}
          OnClick = slinksClick
        end
        object srechts: TSpeedButton
          Left = 62
          Top = 33
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            22002222222222222200}
          OnClick = srechtsClick
        end
        object shoch: TSpeedButton
          Left = 40
          Top = 24
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            22002222222222222200}
          OnClick = shochClick
        end
        object srunter: TSpeedButton
          Left = 40
          Top = 46
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            22002222222222222200}
          OnClick = srunterClick
        end
        object sgroesser: TSpeedButton
          Left = 96
          Top = 33
          Width = 23
          Height = 22
          Hint = 'Intervall Zoom In'
          Flat = True
          Glyph.Data = {
            
            77777777777777770000}
          ParentShowHint = False
          ShowHint = True
          OnClick = sgroesserClick
        end
        object skleiner: TSpeedButton
          Left = 120
          Top = 33
          Width = 23
          Height = 22
          Hint = 'Intervall Zoom out'
          Flat = True
          Glyph.Data = {
            
            77777777777777770000}
          ParentShowHint = False
          ShowHint = True
          OnClick = SkleinerClick
        end
        object Label12: TLabel
          Left = 16
          Top = 8
          Width = 119
          Height = 14
          Caption = 'Coordinate system'
        end
        object sgrundeinstellung: TSpeedButton
          Left = 144
          Top = 33
          Width = 23
          Height = 22
          Hint = 'Zur'#252'cksetzen'
          Flat = True
          Glyph.Data = {
            
            7777777777777777777777777777777777777777777777777777}
          ParentShowHint = False
          ShowHint = True
          OnClick = SgrundClick
        end
      end
      object Panel7: TPanel
        Left = 0
        Top = 70
        Width = 218
        Height = 616
        Align = alClient
        BevelOuter = bvNone
        Color = 15790320
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object Label9: TLabel
          Left = 16
          Top = 440
          Width = 74
          Height = 13
          Caption = 'from             to'
        end
        object Label11: TLabel
          Left = 145
          Top = 250
          Width = 27
          Height = 13
          Caption = ';       )'
        end
        object Label10: TLabel
          Left = 145
          Top = 274
          Width = 27
          Height = 13
          Caption = ';       )'
        end
        object Label13: TLabel
          Left = 104
          Top = 335
          Width = 11
          Height = 13
          Caption = '= '
        end
        object Label2: TLabel
          Left = 16
          Top = 16
          Width = 78
          Height = 13
          Caption = 'Graph width :'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 88
          Top = 140
          Width = 27
          Height = 13
          Caption = '< x <'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Segoe UI'
          Font.Style = []
          ParentFont = False
        end
        object Label1: TLabel
          Left = 16
          Top = 140
          Width = 20
          Height = 13
          Caption = 'DB :'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Segoe UI'
          Font.Style = []
          ParentFont = False
        end
        object Label8: TLabel
          Left = 16
          Top = 176
          Width = 74
          Height = 13
          Caption = 'Parameter P = '
        end
        object Label14: TLabel
          Left = 48
          Top = 355
          Width = 39
          Height = 13
          Caption = 'Area AB'
        end
        object Label15: TLabel
          Left = 104
          Top = 355
          Width = 11
          Height = 13
          Caption = '= '
        end
        object Label16: TLabel
          Left = 138
          Top = 437
          Width = 8
          Height = 16
          Caption = 'D'
          Font.Charset = SYMBOL_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Symbol'
          Font.Style = []
          ParentFont = False
        end
        object Label4: TLabel
          Left = 16
          Top = 200
          Width = 76
          Height = 13
          Caption = 'Parameter Q = '
        end
        object ed_von: TEdit
          Left = 42
          Top = 436
          Width = 32
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 5
          Text = '-6'
        end
        object ed_bis: TEdit
          Left = 98
          Top = 436
          Width = 32
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 6
          Text = '6'
        end
        object b_animation: TButton
          Left = 107
          Top = 540
          Width = 79
          Height = 25
          Caption = 'Animation'
          TabOrder = 1
          TabStop = False
          OnClick = b_animationClick
        end
        object c_punktA: TCheckBox
          Left = 12
          Top = 248
          Width = 74
          Height = 17
          Caption = 'Point A ('
          TabOrder = 18
          OnClick = PaintBox1Paint
        end
        object ed_A: TEdit
          Left = 84
          Top = 246
          Width = 41
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 19
          Text = '1'
        end
        object rb_punkta: TRadioButton
          Left = 104
          Top = 380
          Width = 81
          Height = 17
          Caption = 'Point A'
          TabOrder = 2
        end
        object rb_parameter: TRadioButton
          Left = 104
          Top = 412
          Width = 113
          Height = 17
          Caption = 'Parameter'
          Checked = True
          TabOrder = 4
          TabStop = True
        end
        object c_normale: TCheckBox
          Left = 100
          Top = 296
          Width = 137
          Height = 17
          Caption = 'Normal A'
          TabOrder = 25
          OnClick = PaintBox1Paint
        end
        object c_punktB: TCheckBox
          Left = 12
          Top = 272
          Width = 74
          Height = 17
          Caption = 'Point B ('
          TabOrder = 21
          OnClick = PaintBox1Paint
        end
        object ed_B: TEdit
          Left = 84
          Top = 270
          Width = 41
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 22
          Text = '0'
        end
        object c_tangente: TCheckBox
          Left = 12
          Top = 296
          Width = 88
          Height = 17
          Caption = 'Tangente A'
          TabOrder = 24
          OnClick = PaintBox1Paint
        end
        object c_sekante: TCheckBox
          Left = 12
          Top = 315
          Width = 93
          Height = 17
          Caption = 'Sekante AB'
          TabOrder = 26
          OnClick = PaintBox1Paint
        end
        object c_integral: TCheckBox
          Left = 12
          Top = 334
          Width = 85
          Height = 17
          Caption = 'Integral AB'
          TabOrder = 27
          OnClick = PaintBox1Paint
        end
        object c_1ableitung: TCheckBox
          Left = 12
          Top = 41
          Width = 87
          Height = 17
          Caption = '1.Derivative'
          TabOrder = 9
          OnClick = PaintBox1Paint
        end
        object c_2ableitung: TCheckBox
          Left = 116
          Top = 41
          Width = 95
          Height = 17
          Caption = '2. Derivative'
          TabOrder = 10
          OnClick = PaintBox1Paint
        end
        object c_stammfunktion: TCheckBox
          Left = 12
          Top = 95
          Width = 138
          Height = 17
          Caption = 'Antiderivative function'
          TabOrder = 13
          OnClick = PaintBox1Paint
        end
        object ed_breite: TEdit
          Left = 98
          Top = 11
          Width = 36
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 7
          Text = '1'
          OnChange = TB1Change
        end
        object UpDown1: TUpDown
          Left = 134
          Top = 11
          Width = 16
          Height = 21
          Associate = ed_breite
          Min = 1
          Max = 6
          Position = 1
          TabOrder = 8
        end
        object ed_xvon: TEdit
          Left = 40
          Top = 136
          Width = 41
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 14
          Text = '-1000'
        end
        object ed_xbis: TEdit
          Left = 128
          Top = 136
          Width = 41
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 15
          Text = '1000'
        end
        object ed_parameter: TEdit
          Left = 106
          Top = 171
          Width = 41
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 16
          Text = '1'
        end
        object b_zeichnen: TButton
          Left = 18
          Top = 540
          Width = 79
          Height = 25
          Caption = 'Draw'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Segoe UI'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          TabStop = False
          OnClick = b_zeichnenClick
        end
        object c_nullstellen: TCheckBox
          Left = 12
          Top = 59
          Width = 69
          Height = 17
          Caption = 'Zeroing'
          TabOrder = 11
          OnClick = PaintBox1Paint
        end
        object c_extrema: TCheckBox
          Left = 116
          Top = 59
          Width = 97
          Height = 17
          Caption = 'Extrema'
          TabOrder = 12
          OnClick = PaintBox1Paint
        end
        object UpDown2: TUpDown
          Left = 148
          Top = 169
          Width = 17
          Height = 24
          Min = -10000
          Max = 10000
          TabOrder = 17
          OnChangingEx = UpDown2ChangingEx
        end
        object UpDown3: TUpDown
          Left = 125
          Top = 245
          Width = 17
          Height = 24
          Min = -10000
          Max = 10000
          TabOrder = 20
          OnChangingEx = UpDown3ChangingEx
        end
        object UpDown4: TUpDown
          Left = 125
          Top = 269
          Width = 17
          Height = 24
          Min = -10000
          Max = 10000
          TabOrder = 23
          OnChangingEx = UpDown4ChangingEx
        end
        object c_wendepunkte: TCheckBox
          Left = 12
          Top = 77
          Width = 95
          Height = 17
          Caption = 'Turning point'
          TabOrder = 28
          OnClick = PaintBox1Paint
        end
        object ed_delta: TEdit
          Left = 152
          Top = 436
          Width = 32
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 29
          Text = '0,08'
        end
        object c_wendetangente: TCheckBox
          Left = 116
          Top = 77
          Width = 90
          Height = 17
          Caption = 'Turn tangent'
          TabOrder = 30
          OnClick = PaintBox1Paint
        end
        object c_schnittpunkte: TCheckBox
          Left = 12
          Top = 113
          Width = 85
          Height = 17
          Caption = 'Intersection'
          TabOrder = 31
          OnClick = PaintBox1Paint
        end
        object c_pktanzeige: TCheckBox
          Left = 116
          Top = 113
          Width = 89
          Height = 17
          Caption = 'Point display'
          TabOrder = 32
          OnClick = PaintBox1Paint
        end
        object c_ergebnisliste: TCheckBox
          Left = 40
          Top = 576
          Width = 81
          Height = 17
          Caption = 'Results list'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Segoe UI'
          Font.Style = []
          ParentFont = False
          TabOrder = 33
          OnClick = Bergebnis
        end
        object rb_punktB: TRadioButton
          Left = 104
          Top = 396
          Width = 81
          Height = 17
          Caption = 'Point B'
          TabOrder = 3
        end
        object c_schar: TCheckBox
          Left = 12
          Top = 222
          Width = 93
          Height = 17
          Caption = 'Functions d ='
          TabOrder = 34
          OnClick = PaintBox1Paint
        end
        object ed_q: TEdit
          Left = 106
          Top = 195
          Width = 41
          Height = 21
          CharCase = ecUpperCase
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 35
          Text = '2'
        end
        object UpDown5: TUpDown
          Left = 148
          Top = 193
          Width = 17
          Height = 24
          Min = -10000
          Max = 10000
          TabOrder = 36
          OnChangingEx = UpDown5ChangingEx
        end
        object ed_schardiff: TEdit
          Left = 106
          Top = 220
          Width = 32
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 37
          Text = '0,1'
        end
        object c_wertetabelle: TCheckBox
          Left = 12
          Top = 467
          Width = 173
          Height = 17
          Caption = 'Table of values ??in list'
          TabOrder = 38
          OnClick = PaintBox1Paint
        end
      end
    end
    object Panel4: TPanel
      Left = 0
      Top = 0
      Width = 824
      Height = 686
      Align = alClient
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 1
      object PaintBox1: TPaintBox
        Left = 201
        Top = 0
        Width = 623
        Height = 597
        Align = alClient
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        OnMouseDown = PaintBox1MouseDown
        OnMouseMove = PaintBox1MouseMove
        OnMouseUp = PaintBox1MouseUp
        OnPaint = PaintBox1Paint
      end
      object Panel9: TPanel
        Left = 0
        Top = 597
        Width = 824
        Height = 89
        Align = alBottom
        BevelOuter = bvNone
        Color = 15790320
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object ed_f1: TEdit
          Left = 84
          Top = 12
          Width = 290
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 0
          Text = 'X*SIN(X+P)'
        end
        object ed_f2: TEdit
          Left = 84
          Top = 44
          Width = 290
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 1
          Text = 'X^2/4-2'
        end
        object ed_f3: TEdit
          Left = 456
          Top = 12
          Width = 290
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 2
        end
        object ed_f4: TEdit
          Left = 456
          Top = 44
          Width = 290
          Height = 21
          CharCase = ecUpperCase
          TabOrder = 3
        end
        object button_fstrich: TButton
          Left = 752
          Top = 10
          Width = 48
          Height = 25
          Caption = 'f1'#39'(x)'
          TabOrder = 4
          OnClick = button_fstrichClick
        end
        object c_f1: TCheckBox
          Left = 12
          Top = 16
          Width = 65
          Height = 17
          Caption = 'f1(x) ='
          Checked = True
          State = cbChecked
          TabOrder = 5
          OnClick = b_zeichnenClick
        end
        object c_f2: TCheckBox
          Left = 12
          Top = 48
          Width = 65
          Height = 17
          Caption = 'f2(x) ='
          TabOrder = 6
          OnClick = b_zeichnenClick
        end
        object c_f3: TCheckBox
          Left = 384
          Top = 16
          Width = 65
          Height = 17
          Caption = 'f3(x) ='
          Checked = True
          State = cbChecked
          TabOrder = 7
          OnClick = b_zeichnenClick
        end
        object c_f4: TCheckBox
          Left = 384
          Top = 48
          Width = 65
          Height = 17
          Caption = 'f4(x) ='
          Checked = True
          State = cbChecked
          TabOrder = 8
          OnClick = b_zeichnenClick
        end
        object button_f2strich: TButton
          Left = 752
          Top = 42
          Width = 48
          Height = 25
          Caption = 'f1"(x)'
          TabOrder = 9
          OnClick = button_f2strichClick
        end
      end
      object Mergebnis: TMemo
        Left = 0
        Top = 0
        Width = 201
        Height = 597
        Align = alLeft
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        Visible = False
        WordWrap = False
      end
    end
  end
  object MainMenu1: TMainMenu
    Left = 16
    Top = 16
    object M02: TMenuItem
      Caption = 'File'
      ShortCut = 27
      object Speichern1: TMenuItem
        Caption = 'Save'
        OnClick = Speichern1Click
      end
      object C1: TMenuItem
        Caption = 'Close'
        OnClick = C1Click
      end
    end
    object Bearbeiten1: TMenuItem
      Caption = 'Edit'
      object Vektorgrafikkopieren1: TMenuItem
        Caption = 'Copy vector Graphic'
        OnClick = alsWMFkopieren1Click
      end
      object Vektorgrafikdrucken1: TMenuItem
        Caption = 'Print vector graphic'
        OnClick = GrafikdruckenClick
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object M1: TMenuItem
        Caption = 'Ergebnisliste kopieren'
        Enabled = False
        OnClick = ListekopierenClick
      end
    end
    object M_ksystem: TMenuItem
      Caption = 'Coordinate'
      object MAchsen1: TMenuItem
        Caption = 'Represent axes'
        Checked = True
        OnClick = Menueinstellungen
      end
      object MAchsen21: TMenuItem
        Caption = 'Highlight axes'
        OnClick = Menueinstellungen
      end
      object M_rahmen: TMenuItem
        Caption = 'Coordinate frame'
        OnClick = Menueinstellungen
      end
      object M_winkel: TMenuItem
        Caption = 'Angle measurement on the x-axis'
        OnClick = Menueinstellungen
      end
      object M_bogen: TMenuItem
        Caption = 'Radian measure on x-axis'
        OnClick = Menueinstellungen
      end
      object N7: TMenuItem
        Caption = '-'
      end
      object m_raster: TMenuItem
        Caption = 'Grid'
        Checked = True
        OnClick = Menueinstellungen
      end
      object m_mmraster: TMenuItem
        Caption = 'mm Grid'
        OnClick = Menueinstellungen
      end
      object m_terme: TMenuItem
        Caption = 'Show function terms'
        Checked = True
        OnClick = Menueinstellungen
      end
      object m_farbe: TMenuItem
        Caption = 'Colored function graphs'
        Checked = True
        OnClick = Menueinstellungen
      end
      object N6: TMenuItem
        Caption = '-'
      end
      object intervallverkleinern: TMenuItem
        Caption = 'Reduce display interval'
        OnClick = sgroesserClick
      end
      object intervallvergroessern: TMenuItem
        Caption = 'Increase display interval'
        OnClick = SkleinerClick
      end
      object grundeinstellung: TMenuItem
        Caption = 'Basic setting'
        ShortCut = 32
        OnClick = SgrundClick
      end
      object aufloesunganpassen: TMenuItem
        Caption = 'Adjust y-resolution'
        OnClick = S31Click
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object Animation1: TMenuItem
        Caption = 'Animation'
        ShortCut = 113
        OnClick = b_animationClick
      end
      object Zeichnen1: TMenuItem
        Caption = 'Draw'
        ShortCut = 13
        OnClick = b_zeichnenClick
      end
    end
    object M2: TMenuItem
      Caption = 'Special functions'
      object spiegelungxachse: TMenuItem
        Caption = 'f1(-x) , reflection across the x-axis'
        OnClick = Menueinstellungen
      end
      object spiegelungursprung: TMenuItem
        Caption = '-f1(-x) , reflection at the origin'
        OnClick = Menueinstellungen
      end
      object spiegelungyachse: TMenuItem
        Caption = '-f1(x) , reflection across the y-axis'
        OnClick = Menueinstellungen
      end
      object N4: TMenuItem
        Caption = '-'
      end
      object verschiebungxachse: TMenuItem
        Caption = 'f1(x+p) , shift by -p along the x-axis'
        OnClick = Menueinstellungen
      end
      object streckungxachse: TMenuItem
        Caption = 'f1(p*x) , scaling by p along the x-axis'
        OnClick = Menueinstellungen
      end
      object verschiebungyachse: TMenuItem
        Caption = 'f1(x)+p , shift by p along the y-axis'
        OnClick = Menueinstellungen
      end
      object streckungyachse: TMenuItem
        Caption = 'p*f1(x) , stretching by p along the y-axis'
        OnClick = Menueinstellungen
      end
      object N3: TMenuItem
        Caption = '-'
      end
      object nacheinanderausfuehrung: TMenuItem
        Caption = 'f1(f1(x)) , sequential execution'
        OnClick = Menueinstellungen
      end
      object funktionsaddition: TMenuItem
        Caption = 'f1(x)+f2(x) , addition of the function values'
        OnClick = Menueinstellungen
      end
      object funktionssubtraktion1: TMenuItem
        Caption = 'f1(x)-f2(x) , subtraction of the function values'
        OnClick = Menueinstellungen
      end
      object funktionsmultiplikation: TMenuItem
        Caption = 'f1(x)*f2(x) , multiplication of the function values'
        OnClick = Menueinstellungen
      end
      object funktionsdivision1: TMenuItem
        Caption = 'f1(x)/f2(x) , Division of the function values'
        OnClick = Menueinstellungen
      end
      object N5: TMenuItem
        Caption = '-'
      end
      object Umkehrkurvezuf1x1: TMenuItem
        Caption = 'Inverse curve of f1(x) , reflection across y=x'
        OnClick = Menueinstellungen
      end
    end
  end
  object Timer1: TTimer
    Enabled = False
    Interval = 25
    OnTimer = Timer1Timer
    Left = 48
    Top = 16
  end
  object SD1: TSaveDialog
    DefaultExt = 'gif'
    Filter = 
      'Abbildung (*.bmp)|*.bmp|16 Farben-Abbildung (*.bmp)|*.bmp|Abbild' +
      'ung (*.gif)|*.gif|Vektorgrafik (*.wmf)|*.wmf'
    FilterIndex = 3
    Options = [ofOverwritePrompt, ofHideReadOnly, ofEnableSizing]
    Title = 'Abbildung speichern'
    Left = 82
    Top = 17
  end
end
