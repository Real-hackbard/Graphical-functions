...
uses
  ...
  printers, uableitung, zlib, Gifimage, XPMan;
...
// Helper routine formatted output
...
// Help files for reading the list of functions
...
// Helper routines for combining terms
...
procedure tForm1.koordinatensystem(can:tcanvas);
var k:string;
    sc,sx,sy:integer;
    kodiff:double;
...
// Draw function
procedure tForm1.zeichnen(canvas:tcanvas);
...
// Antiderivative function through O
...
// Drawing routine of a function
procedure funktion(const s2:string);
var x,y,yalt:integer;
    we:double;
// Checking the functional value, including poles
...
begin
  ...
  x:=-2;
  yalt:=-100;
  // Intercepting undefined arguments
  repeat
    inc(x);
    we:=malfyr(funktionswert(s2,subx(x)));
  until (((fehler<>1) and (fehler<>2)) or (x>grafb+10)) and (subx(x)>xdef1);
  // first valid initial value
  if testwe(we) then begin
...
begin
  ...
  if ix<>iy then begin
    ...
    if f2='0' then
      ergebnisliste.add('Area under f1 of ('+_strkomma(ix,1,2)+' | '+_strkomma(iy,1,2)+')')
    else
      ergebnisliste.add('area between f1 and f2 of ('+_strkomma(ix,1,2)+' | '+_strkomma(iy,1,2)+')');
    ergebnisliste.add(#9+_strkomma(sf,1,2));
    if f2='0' then
      ergebnisliste.add('definite integral f1 of ('+_strkomma(ix,1,2)+' | '+_strkomma(iy,1,2)+')')
    else
      ergebnisliste.add('definite integral f1-f2 of ('+_strkomma(ix,1,2)+' | '+_strkomma(iy,1,2)+')');
...
begin
  ...
  repeat
    ...
    inc(x);
  until x>grafh+10;
  if termausgabe then canvas.textout(16,funktionz*16,'Reversal curve');
end;
...
// Find the intersection points f1, f2
...
begin
  ...
  if f5<>'' then begin
    ergebnisliste.add('Intersection points between f1 and f2');
    ergebnisliste.add('Function f2: Y = '+ed_f2.text);
    ix:=a-0.0001;
    iy:=b+0.0001;
    if abs(ix)>100 then ix:=-5.0001;
    if abs(iy)>100 then iy:=5.0001;
...
begin
  ...
  repeat
    s1:=lreadln;
    inc(funktionz);
    ergebnisliste.add('Function Y = '+s1);
    if funktionz=1 then begin
      fabl:=zableitung(s1);
      fabl2:=zableitung(fabl);
      ergebnisliste.add('Area ('+ed_von.text+' | '+ed_bis.text+')');
      ergebnisliste.add('1st derivative');
      ergebnisliste.add('Y = '+fabl);
      ergebnisliste.add('2nd derivative');
      ergebnisliste.add('Y = '+fabl2);
    end;

  // Possibly fill the area
  ...
  if funktionz=1 then begin
    ...
    if c_extrema.checked then begin
      ...
      if extremazahl>0 then begin
        ...
        ergebnisliste.add('Extreme points');
        ...
        canvas.brush.color:=clwhite;
        delete(ns,length(ns)-2,20);
        if c_pktanzeige.checked then canvas.textout(16,grafh-30-funktions*16,'Extreme points: '+ns);
        inc(funktions);
      end;
    end;
    ...
    if c_wendepunkte.checked or c_wendetangente.checked then begin
      ...
      if wendepunktzahl>0 then begin
        ergebnisliste.add('Turning points');
        ns:='';
        ...
        if c_wendepunkte.checked then begin
          delete(ns,length(ns)-2,20);
          if c_pktanzeige.checked then canvas.textout(16,grafh-30-funktions*16,'Turning points: '+ns);
          inc(funktions);
        end;
      end;
    end;
    ...
    if c_schnittpunkte.checked then begin
      ...
      if schnittpunktzahl>0 then begin
        ...
        ergebnisliste.add('Intersection points');
        ...
        canvas.brush.color:=clwhite;
        delete(ns,length(ns)-2,20);
        if c_pktanzeige.checked then canvas.textout(16,grafh-30-funktions*16,'Intersection points: '+ns);
        inc(funktions);
      end;
    end;
    if c_1ableitung.checked then fapaint(s1,'1st derivative',1);
    if c_2ableitung.checked then fapaint(s1,'2st derivative',2);
    // Special functions
    ...
    if (c_wertetabelle.checked) and (Mergebnis.visible) then begin
      ergebnisliste.add('');
      ergebnisliste.add('Table of values');
...
// Routines for moving the coordinate system
procedure TForm1.slinksClick(Sender: TObject);
var b:double;
begin
  b:=(_x2-_x1)/5;
  _x1:=_x1+b;
...
procedure TForm1.srechtsClick(Sender: TObject);
var b:double;
begin
  b:=(_x2-_x1)/5;
  _x1:=_x1-b;
...
procedure TForm1.shochClick(Sender: TObject);
var b:double;
begin
  b:=(_y2-_y1)/5;
  _y1:=_y1-b;
...
procedure TForm1.srunterClick(Sender: TObject);
var b:double;
begin
  b:=(_y2-_y1)/5;
  _y1:=_y1+b;
...
//Zoom-In and Out
procedure TForm1.sgroesserClick(Sender: TObject);
var b:double;
...
procedure TForm1.SkleinerClick(Sender: TObject);
var b:double;
begin
  b:=(_x2-_x1)/10;
  _x1:=_x1-b;
...
// Basic setting
procedure TForm1.SgrundClick(Sender: TObject);
begin
  _x1:=-6;
  _x2:=6;
...
// same resolution in x and y directions
...
procedure TForm1.alsWMFkopieren1Click(Sender: TObject);
...
procedure TForm1.GrafikdruckenClick(Sender: TObject);
var rect:trect;
    breite:integer;
...
procedure TForm1.b_animationClick(Sender: TObject);
begin
  timer1.enabled:=not timer1.enabled;
  if timer1.enabled then b_animation.caption:='Stop'
  else begin
...
// Movement of points A, B / Change in parameter P
...
procedure TForm1.UpDown4ChangingEx(Sender: TObject;
  var AllowChange: Boolean; NewValue: Smallint;
  Direction: TUpDownDirection);
var x:double;
begin
...
// Calculate the first derivative analytically
procedure TForm1.button_fstrichClick(Sender: TObject);
begin
...
// Show results list
procedure TForm1.Bergebnis(Sender: TObject);
begin
...
// Copy result list
procedure TForm1.ListekopierenClick(Sender: TObject);
begin
  Mergebnis.SelectAll;
...
procedure TForm1.Menueinstellungen(Sender: TObject);
begin
  (sender as tmenuitem).checked:=not (sender as tmenuitem).checked;
  paintbox1paint(sender);
end;

procedure TForm1.UpDown5ChangingEx(Sender: TObject;
  var AllowChange: Boolean; NewValue: Smallint;
  Direction: TUpDownDirection);
begin
  q:=ein_double(ed_q);
...
procedure TForm1.button_f2strichClick(Sender: TObject);
begin
