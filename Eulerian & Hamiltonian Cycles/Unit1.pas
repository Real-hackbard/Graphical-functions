...
uses
  ...
  ExtCtrls, Menus, Dialogs, StdCtrls, Grids, Buttons, Mask, XPMan;
...
procedure TForm1.S4C(Sender: TObject);
begin
  if not sabbruch then begin
    sabbruch:=true;
  end
...
procedure TForm1.SLoeschen(Sender: TObject);
begin
  LKnoten.clear;
  LKanten.clear;
  kpunktezahl:=0;
...
procedure TForm1.SLaden(Sender: TObject);
var dd:textfile;
    k:string;
    i:integer;
begin
  if od3.execute then begin
    if r1.itemindex=1 then begin
      hamm:=true;
      Label1.caption:='0 Hamilton circles found'
    end else begin
      Label1.caption:='0 Eulerian circles found';
      hamm:=false;
    end;
    Liste_E.clear;
    LKnoten.clear;
...
procedure TForm1.SSpeichern(Sender: TObject);
var dd:textfile;
    k:string;
    i:integer;
begin
...
procedure TForm1.LB7C(Sender: TObject);
var res:integer;
    k,k1:string;
begin
...
procedure TForm1.B_KnotenC(Sender: TObject);
begin
  if hamm then
    Label1.caption:='0 Hamilton circles found'
  else
...
procedure TForm1.B_LoeschenC(Sender: TObject);
...
procedure TForm1.B_AendernC(Sender: TObject);
var k:string;
    res:integer;
begin
  res:=LKnoten.itemindex;
  if res>=0 then begin
    ...
    k:=chr(64+res+1)+#9+'('+_str2komma(kpunkte[res+1].x)+
          '|'+_str2komma(kpunkte[res+1].y)+')';
    LKnoten.items[res]:=k;
    kp1paint(sender);
  end;
end;

procedure TForm1.B_KanteC(Sender: TObject);
var k1,k2:string;
begin
  if hamm then
    Label1.caption:='0 Hamilton circles found'
...
procedure TForm1.B_KanteLoeschenC(Sender: TObject);
var res:integer;
begin
  if hamm then
    Label1.caption:='0 Hamilton circles found'
...
procedure TForm1.SGroesse(Sender: TObject);
begin
...
procedure TForm1.kp1MD(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var i:integer;
begin
  if button=mbleft then begin
    for i:=1 to kpunktezahl do begin
      if (abs(kpunktegrund[i].x-x)<5) and (abs(kpunktegrund[i].y-y)<5) then
      begin
        grundziehen:=true;
        nummerziehen:=i;
      end;
...
procedure TForm1.kp1MM(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
...
procedure TForm1.kp1MU(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var rx,ry,rpx,rpy:double;
    b,h,j,i:integer;
    k,k1,k2:string;
...
procedure TForm1.kp1Paint(Sender: TObject);
var bitmap:tbitmap;
begin
...
procedure TForm1.xkp1Paint(can:tcanvas);
...
procedure TForm1.B_SuchenC(Sender: TObject);
label 1;
    ...
    if kpunktezahl>1 then begin
      ...
      if kantenzahl>0 then
        ...
        if not hamm then begin
          ...
          if anz>2 then begin
            Liste_E.items.add('No Eulerian circuit possible');
            goto 1;
          end;
          if anz>0 then begin
            for i:=1 to kpunktezahl do
...
1:      setlength(matrix,0,0);
    ...
    sabbruch:=true;
    if hamm then
      B_Suchen.caption:='Search Hamilton district'
    else B_Suchen.caption:='Find Euler circle';
    setzen(true);
    if (Liste_E.items.count=0) or ((Liste_E.items.count=1) and
       (pos('no',Liste_E.items[0])>0)) then begin
      if hamm then begin
        if nurpfad then begin
          Liste_E.items.add('No Hamilton path possible');
          Label1.caption:='0 Hamilton paths found'
        end else begin
          Liste_E.items.add('no Hamilton circle possible');
          Label1.caption:='0 Hamilton circles found'
        end
      end
      else
      ...
      if Liste_E.items.count>250000 then begin
        if hamm then begin
          if nurpfad then Label1.caption:='More than 250,000 Hamilton paths found'
                     else Label1.caption:='More than 250,000 Hamiltonian circles found'
        end
        else
          Label1.caption:='More than 250,000 Eulerian circles found'
      end else begin
        if hamm then begin
          if nurpfad then
            Label1.caption:=inttostr(Liste_E.items.count)+' Hamilton paths found'
          else
            Label1.caption:=inttostr(Liste_E.items.count)+' Hamilton circles found'
        end
        else
          Label1.caption:=inttostr(Liste_E.items.count)+' Eulerian circles found';
      end;
    end;
end;

procedure TForm1.B_AnzeigeC(Sender: TObject);
...
begin
  if not sabbruch then sabbruch:=true
    ...
    setzen(true);
    if hamm then
      B_Anzeige.caption:='View Hamilton Circle'
    else
      B_Anzeige.caption:='Show Euler circle';
  end;
end;

procedure TForm1.CB6C(Sender: TObject);
begin
  if CB_Pfad.checked then panel8.caption:='Hamilton paths'
                     else panel8.caption:='Hamilton circles';
end;

procedure TForm1.Beispielrufen(Sender: TObject);
...
begin
  ...
  B_Suchen.caption:=k+'search circle';
  B_Anzeige.caption:=k+'show circle';
  panel8.caption:=k+'circles';
  Label1.caption:='0 '+k+'circles found';
end;
